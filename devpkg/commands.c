...
int Command_depends(apr_pool_t *p, const char *path)
{
	...
	{
		btrimws(line);
		log_inf("Processing depends: %s", bdata(line));
...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		log_inf("Building according to DEPENDS: %s", url);
		...
		const char *make_opts, const char *install_opts)
{
	...
	if(access(CONFIG_SCRIPT, X_OK) == 0) {
		log_inf("Has a configure script, running it.");
		...
		const char *make_opts, const char *install_opts)
{
	...
	if(rc == 1) {
		log_inf("Package %s already installed.", url);
		return 0;
	}
	...
	if(rc == 1) {
		...
		// no install needed
		log_inf("Depends successfully installed: %s", url);
	} else {
		// had an error
