...
void Database_write(struct Connection *conn)
{
	...
	if(rc != 1) die("Couldn't write database.\n");

	rc = fflush(conn->file);
	if(rc == -1) die("Couldn't flush database file.\n");
}
...
void Database_set(struct Connection *conn, int id, const char*name, const char*email)
{
	...
	if (address->set) die("Address already set. Delete it first.\n");

	address->set = 1;
	char *rc = strncpy(address->name, name, MAX_DATA);
	if (!rc) die("Name copy failed.\n");
	rc = strncpy(address->email, email, MAX_DATA);
	if (!rc) die("Email copy failed.\n");
}
